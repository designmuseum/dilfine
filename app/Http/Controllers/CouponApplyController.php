...
class CouponApplyController extends Controller {
		...
		if ( isset( $cpn ) ) {
			...
			if ( date( 'Y-m-d', strtotime( $cpn->expirydate ) ) >= $today ) {
				...
				if ( $cpn->maxusage != 0 ) {
					...
					return back()->with( 'fail', 'Coupan code max usage limit reached !' );
				}
			...
			Session::forget( 'coupanapplied' );
			Cart::where( 'user_id', Auth::user()->id )->update( [ 'distype' => NULL, 'disamount' => NULL ] );
			return back()->with( 'fail', 'Coupon code is invalid' );
		}

	}

	public
	function validCouponForProduct( $cpn ) {

		if ( Auth::check() ) {
			$cart = Cart::where( 'pro_id', '=', $cpn[ 'pro_id' ] )->where( 'user_id', '=', Auth::user()->id )->first();
			...
			if ( isset( $cart ) ) {
				...
				if ( $cart->pro_id == $cpn->pro_id ) {
					...
					Cart::where( 'pro_id', '=', $cpn[ 'pro_id' ] )->where( 'user_id', '=', Auth::user()->id )->update( [ 'distype' => 'product', 'disamount' => $per ] );
					Cart::where( 'pro_id', '!=', $cpn[ 'pro_id' ] )->where( 'user_id', '=', Auth::user()->id )->update( [ 'distype' => NULL, 'disamount' => NULL ] );

					return back();

				} else {
					return back()->with( 'fail', 'Sorry no valid product found in your cart for this coupon !' );
				}
	...
	public
	function validCouponForCart( $cpn ) {
				...
				if ( $cpn->minamount != 0 ) {
					...
					if ( $total >= $cpn->minamount ) {
						...
						foreach ( $cart as $key => $c ) {
							...
							Cart::where( 'user_id', Auth::user()->id )->update( [ 'distype' => 'cart', 'disamount' => $per ] );
						...
						return back()->with( 'fail', 'Cart total should be ' . $cpn->minamount . ' or greater !' );
					}
					...
					foreach ( $cart as $key => $c ) {
						...
						Cart::where( 'id', '=', $c->id )->update( [ 'distype' => 'cart', 'disamount' => $per ] );
	...
	public
	function validCouponForCategory( $cpn ) {
		...
		if ( Auth::check() ) {
			...
			foreach ( $cart as $row ) {
				...
				if ( $row->product->category->id == $cpn->cat_id ) {
					$catcart->push( $row );
				}
			...
			if ( count( $catcart ) > 0 ) {
				...
				foreach ( $catcart as $key => $c ) {
					...
					Cart::where( 'id', '=', $c->id )->where( 'user_id', Auth::user()->id )->update( [ 'distype' => 'category', 'disamount' => $per ] );
				}
				...
				if ( $cpn->minamount != 0 ) {
					...
					if ( $total > $cpn->minamount ) {
						...
						Cart::where( 'user_id', Auth::user()->id )->update( [ 'distype' => NULL, 'disamount' => NULL ] );
						return back()->with( 'fail', 'This coupon is only available on some category and cart total should be ' . $cpn->minamount . ' or greater for products beloging to those categories' );
					}
