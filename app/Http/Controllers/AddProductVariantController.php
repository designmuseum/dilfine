...
class AddProductVariantController extends Controller {
	...
	public
	function storeCommon( Request $request, $id ) {
		...
		foreach ( $findrows2 as $value ) {
			if ( $request->attr_name2 == $value->attr_name ) {
				return back()->with( 'warning', 'Variant Already Added In Product Variant !' );
			}
		}
		...
		if ( empty( $newcommonvar->cm_attr_val ) ) {
			return redirect()->route( 'add.var', $id )->with( 'warning', "Varients cannot be added, Please add option value before you add varient" );
		}
		$newcommonvar->pro_id = $id;

		$newcommonvar->save();

		return redirect()->route( 'add.var', $id )->with( 'added', "Variant Added Successfully !" );
	}

	public
	function updatecommon( Request $request, $id ) {
		$cvar = CommonVariants::find( $id );

		if ( isset( $cvar ) ) {
			$cvar->cm_attr_val = $request->cm_attr_val;
			if ( empty( $cvar->cm_attr_val ) ) {
				return redirect()->route( 'add.var', $id )->with( 'added', "Varients cannot be added, Please add option value before you add varient" );
			}
			$cvar->save();
			return back()->with( 'updated', 'Common variant option updated !' );
		} else {
			return back()->with( 'warning', '404 Not found !' );
		}
	}

	public
	function delCommon( Request $request, $id ) {
		$cmvar = CommonVariants::findorfail( $id );

		$cmvar->delete();

		return redirect()->route( 'add.var', $cmvar->pro_id )->with( 'deleted', "Variant Deleted Successfully !" );
	}
	...
	function store( Request $request, $id ) {
		...
		foreach ( $findrows2 as $value ) {
			if ( $request->attr_name == $value->cm_attr_id ) {
				return back()->with( 'warning', 'Variant Already Added In Common Variant !' );
			}
		}
		...
		if ( $findrows->count() >= 2 ) {
					...
					foreach ( $update as $newup ) {
						...
						if ( count( $value ) <= 1 ) {
							...
							foreach ( $value as $cval ) {
								...
								DB::table( 'add_sub_variants' )->where( 'id', $newup->id )->update( array(
									'main_attr_id' => $str3
								) );
			...
			foreach ( $findallsub as $value ) {
				...
				foreach ( $nArry2 as $key => $n ) {
					...
					$new1 = json_encode( $n );
					DB::table( 'add_sub_variants' )->where( 'id', $value->id )->update( array(
	...
	public
	function getProductValues( Request $request ) {
	...
	function destroy( $id ) {
		...
		$getallsub = AddSubVariant::where( 'pro_id', $findpro->pro_id )->get();
	...
	function update( Request $request, $id ) {
		...
		return redirect()->route( 'add.var', $findpro->pro_id )->with( 'updated', 'Product Values Updated Successfully !' );
	}
}