...
class GuestController extends Controller
{
    ...
    public function showpage($slug)
    {
        ...
        SEOTools::setDescription(strip_tags($page->des));
    ...
    public function awb(Request $request, $id)
    {
        ...
        if (empty($id) && empty($email) && empty($awb)) {
            return abort(404, 'Page Not found');
        }
        
        if(!empty($email) && !empty($awb)){
            $o = new \stdClass();
            $o->awb = $awb;
            $o->email = $email;
        } else {
            $decodedData = Crypt::decryptString($id);
            $o = json_decode($decodedData);
        }
        
        if($o->awb == "" || $o->email == ""){
            return abort(404, 'Page Not found');
        } else {
            $user = User::where('email', $o->email)->firstOrFail();
            $invoiceDownloads = InvoiceDownload::where('awb_no', $o->awb)->firstOrFail();
            Order::where('id', $invoiceDownloads->order_id)->where('user_id', $user->id)->firstOrFail();
        }
        require_once('price.php');
        ...
        if ($tokenResponse->ok()) {
            ...
            ])->get("https://apiv2.shiprocket.in/v1/external/courier/track/awb/$id");
            if($trackResponse->ok()){

            }*/

            $trackResponse = Http::get("https://kr-multichannel.s3.ap-southeast-1.amazonaws.com/webhook/docs/sample.json");
            if ($trackResponse->ok()) {
                $track_data = $trackResponse->json();
            }

        }
        return view('front.awb', compact('conversion_rate', 'track_data'));
    }
    
    public function package(){
        $random = Str::random(40);
        require_once('price.php');
        return view('front.package', compact('conversion_rate', 'random'));
    }
}
