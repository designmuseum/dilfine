...
class CheckoutController extends Controller
{
    ...
    public function trackorder($id)
    {
		...
		$orderDetails['pickup_postcode'] = 145023;
		$orderDetails['delivery_postcode'] = 500068;
		$orderDetails['delivery_postcodedelivery_postcode'] = "0.2";
        if ($tokenResponse->ok()) {
            ...
            ])->get("https://apiv2.shiprocket.in/v1/external/courier/serviceability/",$orderDetails);
			print_r($trackResponse->json());die;
//            if($trackResponse->ok()){
//
//            }
//
//            $trackResponse = Http::get("https://kr-multichannel.s3.ap-southeast-1.amazonaws.com/webhook/docs/sample.json");
            if ($trackResponse->ok()) {
                $track_data = array('error'=>1,"data"=>$trackResponse->json());
            }else{
				$track_data = array('error'=>0,"data"=>$trackResponse->json());
			}
