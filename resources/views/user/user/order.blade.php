                  ...
                  <div class="panel-body">
                        ...
                        @foreach($firstarray as $o)
                          ...
                          <div class="row rowbox no-pad">
                            ...
                            @if($o->status == 'shipped')
                                <br>
                                <a href="javascript:trackPackage('{{$o->awb_no}}')" >
                                    <span class="badge badge-info"><span class="fa fa-thumb-tack" aria-hidden="true"></span> Track your package</span>
                                </a>
                            @endif
                            </div>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
   ...
   <div class="track_container" id="dialog" title="Track Package"></div>
@endsection
@section('script')
  ...
  <script src="{{ url('js/userorder.js') }}"></script>
    <script>
        ...
        function trackPackage(awb){
            $('#dialog').html('');
            $.getJSON("{{url('trackorder')}}/"+awb, function(data) {
              let content = '<ul class="list-group">'
                +'<li class="list-group-item">awb :: ' + data.awb + '</li>'
                +'<li class="list-group-item">current_status :: ' + data.current_status + '</li>'
                +'<li class="list-group-item">current_timestamp :: ' + data.current_timestamp + '</li>'
                +'<li class="list-group-item">order_id :: ' + data.order_id + '</li>'
                +'<li class="list-group-item">etd :: ' + data.etd + '</li>'
                if(data.scans.length > 0){
                    content = content + '<li class="list-group-item">scans<ul class="list-group">';
                    $.each( data.scans, function( i, item ) {
                        content = content + '<li class="list-group-item">'
                            +'<ul class="list-group">'
                                +'<li class="list-group-item">date :: ' + item.date + '</li>'
                                +'<li class="list-group-item">activity :: ' + item.activity + '</li>'
                ...
                content = content + '</ul>';
                $('#dialog').html(content);
            }).done(function(){dialog.dialog("open")});
        }
    </script>
