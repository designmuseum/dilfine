      ...
      <tr>
        ...
        <td>
          ...
          <select class="js-example-basic-multiple form-control pay_m" id="payment_checkout{{$key}}" name="payment[]"
            ...
            <option @if(!empty($cury->checkoutCurrencySettings)) @foreach($payments as $pay){{$pay=='wallet' ? 'selected':''}}@endforeach
            ...
            <option @if(!empty($cury->checkoutCurrencySettings)) @foreach($payments as $pay){{$pay=='braintree'?'selected':''}}@endforeach
